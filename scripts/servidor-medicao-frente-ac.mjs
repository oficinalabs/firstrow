...
/*
 * Arranca o servidor de produção contra a BASE DE TESTES DESTA FRENTE, na 3021.
 *
 *   node scripts/servidor-medicao-frente-ac.mjs
 *
 * É o gémeo do `servidor-medicao-frente-y.mjs` (3017) e existe separado pela
 * mesma razão: várias frentes correm em paralelo, e duas a disputar a mesma
 * porta e a mesma base semeavam-se uma à outra a meio da medição.
 *
 ...
 * dados. A guarda abaixo recusa qualquer nome que não pareça descartável.
 */
...
console.info(`Servidor a abrir a base: ${nome} (porta 3021)`);

spawn("./node_modules/.bin/next", ["start", "-p", "3021"], {
  stdio: "inherit",
  env: {
    ...process.env,
    ...env,
